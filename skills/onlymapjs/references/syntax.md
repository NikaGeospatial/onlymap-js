...
- A REAL PROJECTION is `crs` on `<om-map>`: `<om-map crs="EPSG:5070" basemap="none" center="[-98,39]" zoom="4">`. The whole renderer draws in that plane — parallels curve, an equal-area projection is genuinely equal-area. Web Mercator and plate carrée need nothing; national grids and UTM zones resolve through proj4, loaded only when a map asks; anything else takes `crs-def="+proj=…"` or `OmMap.registerCrs(code, defOrFunctions)`.
  - THE AUTHORED CAMERA STAYS GEOGRAPHIC. `center` is lng/lat and `zoom` means what it always meant, so stories, `seek()`, `getCamera()` and state bridges are unchanged. Clicks report lng/lat, not plane metres.
  - Limits, each a validation ERROR with a fix, never a silent no-op: no MapLibre basemap (`basemap="none"`), no `terrain`, no `pitch`/`bearing` (the view is planimetric), no H3/S2/geohash/quadkey layers (deck builds their geometry from a token inside the layer — convert the cells to polygons), and the CRS is fixed for the map's life.
  - Relief DOES work on a projected sheet, both routes. `sun-datum="sheet"` (default) keeps the apparent light constant across the plate; `"geographic"` is physically true. The swing is 34° across a CONUS sheet; the attribute is inert in Mercator and the validator says so.
  - `reference-scale="1:50000"` (or `"50m"`) is what millimetre-authored `<om-transform>` knobs resolve against.
- GEOMETRY TRANSFORMS are `<om-transform>` CHILDREN of an `<om-layer>`, applied in document order, in the DATA stage — so deck, `anchor="surface"`, the legend and the vector exporter all see the same geometry.
  - `type="simplify" tolerance="0.15mm"` — Douglas–Peucker, measured on the GROUND rather than in degrees (a longitude degree is 111 km at the equator and 56 km at 60°N).
  - `type="smooth" iterations="2"` — Chaikin, 1–6. One pass removes the straight-segment look, which is the loudest "web map" tell there is.
  - `type="dot-density" field="population" per="1000"` — seeded (`seed=`, `max=`), so a recorded story and a live map place the same dots. Replaces the row set with points carrying the source row's properties.
  - `type="buffer" distance="0.9mm" repeat="5" fade` — bands around a shape, outermost first, each carrying `_band` and `_bandOpacity` (read it: `get-fill-color="[125,155,173, $_bandOpacity * 165]"`). N bands are N thresholds of ONE signed distance field, so five cost what one costs and the output cannot self-intersect. An ORNAMENT with a measured half-cell bound (0.042 mm at 300 dpi), NOT an analytic setback — for a distance somebody will act on, point at PostGIS/turf.
  - `type="contour" src="terrarium" bounds="[w,s,e,n]" interval="100m" zoom="10"` — isolines marched off a DEM (not deck's `ContourLayer`, which contours point DENSITY). It SOURCES geometry onto a layer with no data; its extent is DECLARED rather than taken from the camera, because a transform runs once at ingest. Each line carries `elevation` and `index` (true every fifth line — the cartographer's heavier contour). The interval is a ground height; `interval="2mm"` is an error.
...
- A LABEL THAT FOLLOWS ITS LINE is `<om-layer type="TextOnPathLayer" get-path="$path" get-text="$name">` — a river's name along the river, a range's along the range. deck has no such layer; this is a composite over `TextLayer` emitting one instance per glyph, each rotated to the local tangent.
  ...
  - Placement is computed in SCREEN PIXELS and re-done when the camera moves, because a glyph advance is pixels while a path is world distance. On a print sheet the camera is fixed, so it resolves once.
  - The SVG export emits ONE `<textPath>` per label rather than per-glyph `<text>` — the one place the exporter produces better output than the screen.
- VECTOR EXPORT is `await map.snapshot({ format: "svg" })`, returning SVG TEXT (`as: "blob"` for a file, `effectDpi` for the raster bands, `onNotice` for dev notices). Polygons and paths become `<path>`, labels become real `<text>` after a deterministic CPU decluttering pass, and anything whose appearance is pixels becomes a placed `<image>` band. Consecutive raster layers merge into one band; a vector layer between them keeps them apart. A `<om-effect>` print finish treats the FINISHED frame, so a sheet carrying one exports as a single raster band at `effectDpi` (default 600) and says so — tell the user that before they build a sheet around both. A decluttering layer does NOT trigger it. `await frame.freeze({format: "svg"})` stores that SVG inline in a cartograph frame, placed by the same `crs` + `corners`; the frozen sheet opens with no runtime and nothing defining its custom elements.
