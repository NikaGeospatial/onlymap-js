...
`crs` puts the whole renderer in a real projection's plane. Parallels curve, an equal-area projection is genuinely equal-area, and a state-plane sheet is drawn in state plane rather than stretched out of Web Mercator:

```html
<om-map crs="EPSG:5070" reference-scale="1:6500000" basemap="none" center="[-96.8, 38.6]" zoom="3.8">
  ...
  <om-layer id="relief" type="HillshadeLayer" src="terrarium" sun-datum="sheet"></om-layer>
</om-map>
```

**The authored camera stays geographic.** `center` is lng/lat and `zoom` means what it always meant, so stories, `seek()`, `getCamera()` and every state bridge keep working; clicks report lng/lat rather than plane metres, which matters because a projected metre in a geodesic formula returns a confident wrong distance. Web Mercator and plate carrée need nothing; the national grids and UTM zones load proj4 only when a map asks for one; anything else takes `crs-def="+proj=…"` or `registerCrs()`. What a projected map can't do — MapLibre basemaps, `terrain`, pitch, spatial-index layers — is a validation error with a fix, never a silent no-op.

**Relief works on a projected sheet**, both ways: a DEM of your own has no projection tie at all, and the keyless global `HillshadeLayer` re-indexes its Mercator tiles from the plane's own geography and draws them as subdivided warped meshes (measured sub-pixel — 0.40 output pixels for Albers at z=3, falling as 1/N²). `sun-datum` picks which north the sun means: sheet north by default, because geographic north swings 34° across a CONUS plate and light that rotates reads as a mistake.

**`<om-transform>` adds geometry the data doesn't contain** — `simplify`, `smooth`, `dot-density`, `buffer`, `contour` — in the data stage, so deck, `anchor="surface"`, the legend and the vector exporter all see the same shape. Spatial knobs are millimetres on paper, resolved against a declared `reference-scale`; ground units (`tolerance="50m"`) need none. `buffer` with `repeat` and `fade` is the engraved shoreline of an old chart: five bands are five thresholds of ONE signed distance field, so they cost what one costs and cannot self-intersect. It is an ornament with a measured half-cell bound, not an analytic setback — asking for the latter is an error naming PostGIS.

**Labels can follow the line they name.** `<om-layer type="TextOnPathLayer">` runs a river's name along the river and a range's along the range — one text instance per glyph, each rotated to the local tangent. It reads left to right whichever way the line was digitised, and a name that does not fit its line is dropped rather than squeezed. In the SVG export it becomes a single `<textPath>`: one editable word still attached to the river's own path.

**`snapshot({ format: "svg" })` exports vector.** Polygons and paths become `<path>`, labels become real `<text>` after a deterministic CPU decluttering pass, and anything whose appearance is pixels becomes a placed raster band — vector linework over relief, which is how cartographic production has always worked. A print finish treats the finished frame, so a sheet carrying one exports as a single band at `effectDpi` (default 600) and says so rather than letting you find out at a print shop. `frame.freeze({format: "svg"})` stores that SVG inline in a cartograph frame, placed by the same `crs` + `corners`: a frozen sheet opens with no runtime, no network, and nothing defining its custom elements. Example: `examples/features/cartography/compose-an-atlas-plate.html`. Guide: [docs/cartography.md](docs/cartography.md).
